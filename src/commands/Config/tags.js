...
> tags delete [nombre]`)
        switch (args[0].toLowerCase()) {
            case 'add': {
                ...
                let message = args.slice(2).join(' ').match(/{message:(.|\d)+}/gi) ? args.slice(2).join(' ').match(/{message:(.|\d)+}/gi)[0].split(':')[1].slice(0, -1) : null
                let embed_name = args.slice(2).join(' ').match(/{embed:(.|\d)+}/gi) ? args.slice(2).join(' ').match(/{embed:(.|\d)+}/gi)[0].split(':')[1].slice(0, -1) : null
                let addRoleID = args.slice(2).join(' ').match(/{addrole(:(.|\d)+}){1,2}/gi) ? args.slice(2).join(' ').match(/{addrole(:(.|\d)+}){1,2}/gi)[0].split(':').slice(1).map((r) => msg.guild.roles.resolve(r)) : null
                let deleteRoleID = args.slice(2).join(' ').match(/{removerole(:(.|\d)+}){1,2}/gi) ? args.slice(2).join(' ').match(/{removerole(:(.|\d)+}){1,2}/gi)[0].split(':').slice(1).map((r) => msg.guild.roles.resolve(r)) : null
